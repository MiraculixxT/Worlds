...
object ChunkScans {
        ...
        regions.forEachIndexed { done, region ->
            ...
            if (table != null) {
                ChunkRegions.forEachChunk(region) { pos ->
                    ...
                    if (stamp != 0) stamps[pos.toLong()] = stamp.toLong()
                }
            }
    ...
    private fun scanFields(
        ...
        eachChunk(dimension, SUB_REGION, regions, onProgress) { store, pos ->
            ...
            if (tag.contains("InhabitedTime")) inhabited[packed] = tag.getLong("InhabitedTime")
            if (tag.contains("LastUpdate")) lastUpdate[packed] = tag.getLong("LastUpdate")
            if (tag.contains("DataVersion")) dataVersion[packed] = tag.getInt("DataVersion").toLong()
        }
        return mapOf(
    ...
    private fun scanEntities(
        ...
        eachChunk(dimension, SUB_ENTITIES, regions, onProgress) { store, pos ->
            val tag = store.read(pos) ?: return@eachChunk
            counts[pos.toLong()] = tag.getListOrEmpty("Entities").size.toLong()
        }
    ...
    private fun scanBlocks(
        ...
        eachChunk(dimension, SUB_REGION, regions, onProgress) { store, pos ->
            ...
            tag.getListOrEmpty("sections").forEach { entry ->
                val states = (entry as? CompoundTag)?.get("block_states") as? CompoundTag
                if (states != null) total += countBlocks(states, filter)
            }
            // Also store & display 0 values
            counts[pos.toLong()] = total
        }
        return mapOf(ChunkMetric.BLOCK_COUNT to counts)
    }
    ...
    private fun countBlocks(states: CompoundTag, filter: BlockFilter): Int {
        ...
        val data = states.getLongArray("data")
        if (data.isEmpty()) return if (matching[0]) BLOCKS_PER_SECTION else 0
    ...
    private fun surfaceBiome(tag: CompoundTag, minY: Int): String? {
        ...
        val section = tag.getListOrEmpty("sections").firstOrNull { entry ->
            (entry as? CompoundTag)?.takeIf { it.contains("Y") }?.getByte("Y")?.toInt() == y shr 4
...
private class Heightmap(private val packed: LongArray, private val perLong: Int, private val bits: Int) {
    ...
    companion object {
        fun of(tag: CompoundTag): Heightmap? {
            val packed = (tag.get("Heightmaps") as? CompoundTag)
                ?.getLongArray("MOTION_BLOCKING") ?: return null
    ...
    fun number(root: CompoundTag, path: String): Long? {
        ...
        path.split('.').forEach { raw ->
            ...
            INDEX.findAll(raw).forEach { match ->
                ...
                current = list[index] as net.minecraft.nbt.Tag
            }
        }
        return when (val tag = current) {
            is NumericTag -> tag.asLong
