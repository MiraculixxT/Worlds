    ...
    companion object {
        ...
        private fun tagMembers(
            ...
            MultiPackResourceManager(PackType.SERVER_DATA, repository.openAllSelected()).use { manager ->
                val tags = TagLoader.loadTagsForRegistry(
                    manager, Registries.BLOCK,
                    TagLoader.ElementLookup.fromGetters(
                        Registries.BLOCK, BuiltInRegistries.BLOCK, BuiltInRegistries.BLOCK,
                    ),
                )
                val members = tags[TagKey.create(Registries.BLOCK, id)]
                if (members == null) {
                    Constants.LOG.warn("Unknown block tag #{}", id)
                    emptySet()
                } else {
                    members.mapNotNullTo(HashSet()) { holder ->
                        holder.unwrapKey().orElse(null)?.identifier()?.toString()
                    }
                }
            }
        } catch (e: Exception) {
