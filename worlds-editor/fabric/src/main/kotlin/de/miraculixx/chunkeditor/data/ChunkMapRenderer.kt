    ...
    private fun shade(colors: IntArray, heights: IntArray, depths: IntArray, pixels: Int, step: Int): IntArray {
        ...
        for (z in 0 until pixels) {
            for (x in 0 until pixels) {
                ...
                // What MapColor.calculateARGBColor does; 1.21's FastColor has no scaleRGB
                out[i] = scaleRgb(mapColor.col, brightness.modifier)
            }
        }
        return out
    }

    /** Vanilla's own per-channel brightness scaling, written out because 1.21 has no `scaleRGB`. */
