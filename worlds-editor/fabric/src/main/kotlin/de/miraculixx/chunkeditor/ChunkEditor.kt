...
object ChunkEditor {
    fun open(parent: Screen, access: LevelStorageSource.LevelStorageAccess) {
        ...
        Minecraft.getInstance().setScreen(ChunkMapScreen(parent, backend))
    }
    ...
    fun openOwned(parent: Screen, levelId: String) {
        ...
        minecraft.setScreen(ChunkMapScreen(parent, backend) { access.safeClose() })
    }
    ...
    fun openRemote(parent: Screen) {
        ...
        minecraft.setScreen(
            ConfirmScreen(
                { confirmed ->
                    if (!confirmed) return@ConfirmScreen minecraft.setScreen(parent)
    ...
    private fun openConfirmed(parent: Screen, hello: Hello, local: Boolean) {
        ...
        Constants.SCOPE.launch {
            val fresh = runCatching { Bodies.readHello(ClientNet.request(C2S.OPEN, ByteArray(0))) }
                ...
                .getOrDefault(hello)
            minecraft.execute { minecraft.setScreen(ChunkMapScreen(parent, RemoteBackend(fresh))) }
        }
    }
