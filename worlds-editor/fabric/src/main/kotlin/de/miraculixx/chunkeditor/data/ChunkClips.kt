...
object ChunkClips {
    ...
    internal fun storage(dir: Path, sub: String, dimension: ResourceKey<Level>, create: Boolean): RegionStore? =
        ChunkRegions.storage(dir.resolve(sub), dir.name, dimension, sub, create)

    fun dimensionKey(id: String): ResourceKey<Level> =
        ResourceLocation.tryParse(id)?.let { ResourceKey.create(net.minecraft.core.registries.Registries.DIMENSION, it) }
            ?: Level.OVERWORLD
}
