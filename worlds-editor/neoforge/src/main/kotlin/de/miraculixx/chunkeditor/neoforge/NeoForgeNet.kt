...
object NeoForgeNet : Transport {
    ...
    fun register(event: RegisterPayloadHandlersEvent) {
        ...
        if (FMLEnvironment.dist == Dist.CLIENT) NeoForgeClientNet.register(registrar)
        // 21.1's registrar has no handler-less overload, and a dedicated server never receives one
        else registrar.playToClient(EditorPacket.S2C_TYPE, EditorPacket.S2C_CODEC) { _, _ -> }
        Net.transport = this
    }
