...
object NativeDialogs {
    ...
    /** The library folder may not be there yet, so open at the nearest parent that is */
    private fun existing(start: Path): Path? =
        generateSequence(start.toAbsolutePath()) { it.parent }.firstOrNull { Files.isDirectory(it) }

    private fun show(
        title: String, start: Path?, onPicked: (Path?) -> Unit, dialog: (String, MemoryStack) -> String?,
    ) {
        val minecraft = Minecraft.getInstance()
        // tinyfd opens where the trailing separator points, and only `title` may be null (`nUTF8Safe`)
        val where = start?.let { existing(it) }?.let { "$it${File.separator}" } ?: ""
        scope.launch {
            val picked = try {
